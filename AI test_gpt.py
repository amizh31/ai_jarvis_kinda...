# Full Jarvis-style GUI with hotkeys, sound activation, fade-in animation, and proper output routing
from tkinter import Tk, Text, Frame, Label, StringVar, ttk, messagebox
...
def play_jarvis_activation_sound():
    wave_obj = sa.WaveObject.from_wave_file('Jarvis ping.wav')
...
class AIApp:
    def __init__(self, root):
        ...
        self.code_output_frame = Frame(root, bg="#1a1a1a", width=384)
        self.code_output_frame.pack(side="left", fill="y")
        self.code_output_frame.pack_propagate(False)

        self.code_output_text = Text(self.code_output_frame, bg="#000000", fg="#00ff99", insertbackground="#00ff99",
                                     font=("Consolas", 11), wrap="word")
        self.code_output_text.pack(fill="both", expand=True)

        self.main_output_text = Text(root, bg="#111a22", fg="#f0f0f0", insertbackground="#00ffe0",
                                     font=("Courier New", 12), wrap="word")
        self.main_output_text.pack(side="left", fill="both", expand=True)
        self.main_output_text.pack_forget()

        self.sidebar = Frame(root, bg="#141924", width=384)
        ...
        self.speech_var = StringVar(value="text")
        self.speech_radio_frame = Frame(self.sidebar, bg="#141924")
        ...
        self.input_label = Label(self.sidebar, text="Enter your query:", fg="#00ffe0", bg="#141924", font=("Consolas", 12, "bold"))
        self.input_label.pack(pady=(30, 5), padx=20, anchor="w")

        self.input_text = Text(self.sidebar, height=5, bg="#2b2b2b", fg="#ffffff", insertbackground="#00ffe0", font=("Consolas", 11))
        self.input_text.pack(padx=20, fill="x")
        self.input_text.bind("<Shift-Return>", lambda event: self.process_input())
        ...
        self.intro_label = Label(self.sidebar, text="Spoken Intro:", fg="#00ffe0", bg="#141924", font=("Consolas", 12, "bold"))
        self.intro_label.pack(pady=(20, 5), padx=20, anchor="w")

        self.spoken_intro_text = Text(self.sidebar, height=5, bg="#0f0f0f", fg="#00ffcc", insertbackground="#00ffcc",
